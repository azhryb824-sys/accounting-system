...
تجيب بالعربية بوضوح واختصار، وتركز على المحاسبة والفواتير والمخزون والقيود اليومية.
إذا كان السؤال خارج نطاق المحاسبة، أجب بما تعرفه دون ادعاء معلومات غير مؤكدة.
""".strip()

PRIVATE_KNOWLEDGE = {
    "الفاتورة الضريبية": "الفاتورة الضريبية هي مستند رسمي يثبت عملية البيع ويعرض بيانات البائع والمشتري والسلع أو الخدمات والمبلغ وضريبة القيمة المضافة.",
    ...
    "الدفع النقدي": "الدفع النقدي يعني أن قيمة العملية تم تحصيلها مباشرة وقت البيع أو تقديم الخدمة، بدل تسجيلها كذمة على العميل.",
    "البيع الآجل": "البيع الآجل يعني بيع سلعة أو خدمة الآن مع تأجيل تحصيل المبلغ، ويظهر عادة ضمن حسابات العملاء أو الذمم المدينة.",
    "ضريبة القيمة المضافة": "ضريبة القيمة المضافة هي ضريبة غير مباشرة تفرض على السلع والخدمات الخاضعة لها، وتظهر في الفاتورة حسب النسبة المعتمدة.",
    "حد التنبيه": "حد التنبيه في المخزون هو مستوى تحدده للصنف حتى ينبهك النظام عند انخفاض الكمية، مما يساعدك على إعادة الطلب في الوقت المناسب.",
    "من أنت": f"أنا {MODEL_NAME}، مساعد ذكاء اصطناعي محاسبي خاص بـ {MODEL_OWNER} ومصمم لمساعدتك في الفواتير والمخزون والقيود اليومية.",
}
...
ACCOUNTING_PATTERNS = [
    ...
    (
        ("سلفة", "سلف", "advance"),
        "سلفة الموظف تُسجل كأصل على حساب سلف الموظفين عند صرفها. وعند خصمها من الراتب ينخفض رصيد السلفة ويظهر الخصم ضمن قيد استحقاق الراتب حتى تصبح السلفة مسددة بالكامل.",
    ),
    (
        ("فاتورة بيع", "مبيعات", "بيع", "عميل"),
        "فاتورة البيع تؤثر محاسبيا على الإيرادات وضريبة القيمة المضافة. إذا كانت نقدية أو بطاقة أو تحويل يكون الطرف المدين الصندوق أو البنك، وإذا كانت آجلة يكون الطرف المدين العملاء. كما ينخفض المخزون وتثبت تكلفة البضاعة المباعة عند ترحيل الفاتورة.",
    ),
    (
    ...
    (
        ("زكاة", "ضريبة", "vat", "القيمة المضافة"),
        "ضريبة القيمة المضافة تظهر في المبيعات كالتزام مستحق للجهة الضريبية، وتظهر في المشتريات كضريبة مدخلات. صافي الضريبة يعتمد على الفرق بين ضريبة المخرجات وضريبة المدخلات.",
    ),
]
...
class PrivateAccountingModel:
    def __init__(self, model_path: Path = MODEL_PATH):
        ...
        if not self.model_path.exists():
            raise FileNotFoundError(f"لم يتم العثور على النموذج في: {self.model_path}")
    ...
    def answer(self, question: str, max_new_tokens: int = 120) -> str:
        ...
        if not self._is_usable_arabic_answer(answer):
            return (
                "هذا السؤال يحتاج تدريبًا إضافيًا داخل النموذج الخاص. "
                "أضف مثالًا مشابهًا في بيانات التدريب ثم شغل train.py لتحسين الإجابة."
            )
        return answer
