...
class InvoiceAccountingTests(TestCase):
    ...
    def test_ai_assistant_template_has_valid_voice_patterns(self):
        ...
        self.assertIn("loadStaticAudioSamples", template)
        self.assertIn("appendStaticVoicesToMainSelect", template)
        ...
        self.assertIn("صوت شبابي", template)
        self.assertIn("صوت طفولي", template)
        self.assertIn("profilePitchBoost", template)
        self.assertIn("const maxChunkLength = 130", template)
        self.assertIn("chunks.slice(0, 60)", template)
